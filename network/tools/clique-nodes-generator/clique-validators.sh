...
if [ -z "$docker_image" ]; then
  docker_image=nethermind/nethermind:1.28.0
fi
...
main() {
  ...
  for i in $(seq 1 $validators); do mkdir -p validator$i; done
  ...
  for i in $(seq 1 $validators); do mkdir -p validator$i/configs validator$i/staticNodes; done

  writeEmptyStaticNodesFile $i

  for i in $(seq 1 $validators); do
    PORT=$((30301 + $i))
    PRIVATE_IP=172.16.239.$((10 + $i))
  ...
  for i in $(seq 1 $validators); do
    ...
    until $(curl --output /dev/null -sf localhost:$JSONRPC_PORT); do
      if [ ${ATTEMPT} -eq ${MAX_ATTEMPTS} ]; then
        printf "\nCouldn't reach one of the JSON RPC endpoints. Something is wrong with the validator$i\n"
        break
      fi
      printf '.'
      ATTEMPT=$(($ATTEMPT + 1))
    ...
    printf "\nStatic node for validator$i: $STATIC_NODE\n"
...
function writeNethermindConfig() {
  cat <<EOF >validator$1/configs/config.cfg
{
    "Init": {
        "StoreReceipts" : true,
  ...
  cat <<EOF >>docker-compose.yml
    validator$1:
        ...
        volumes:
            ...
            - ./validator$1/configs/config.cfg:/nethermind/configs/config.cfg
            - ./static-nodes.json:/nethermind/Data/static-nodes.json
            - ./validator$1/db/clique:/nethermind/nethermind_db/clique
            - ./validator$1/keystore:/nethermind/keystore
        ports:
            - 0.0.0.0:$2:8545
        networks:
            vpcbr:
...
function readSigners() {
  log=$(docker logs validator$1 | grep Node)
...
function clearDbs() {
  for i in $(seq 1 $validators); do
    printf "Clearing db of validator$i\n"
    # sudo because they are created by docker
    sudo rm -rf validator$i/db/clique
  done
}
